/* =====================================================================
   German Global Accounting LLC — site behaviour
   ---------------------------------------------------------------------
   Contents
     1. Language engine (the important one)
     2. Language switcher UI (segmented control with a sliding pill)
     3. Mobile navigation
     4. Scroll state: header, back-to-top ring, process beam
     5. Scroll reveals + heading polish
     6. Pointer effects: card glare + tilt
     7. Contact form
   ===================================================================== */
(function () {
  "use strict";
  ...
  /* ===================================================================
     ...
     Every user-visible string carries a data-i18n key. Attributes are
     handled through data-i18n-attr, e.g.

        data-i18n="form_name_ph" data-i18n-attr="placeholder"

     A missing key falls back to English rather than leaving whatever text
     happened to be on screen — that fallback is what prevents one language
  ...
  function readStoredLang() {
    // An explicit ?lang= wins, so a shared link always opens in the language
    // it was shared in, whatever the recipient chose last time.
  ...
  /* WhatsApp links carry a greeting in the visitor's own language. */
  ...
  /* Readex Pro (Arabic) is only downloaded when Arabic is actually shown. */
  ...
  function applyLanguage(lang, opts) {
    ...
    if (animate) {
      /* Staggered blur-wipe: text nearer the top of the viewport swaps first,
         so the change ripples down the screen instead of blinking all at once. */
      document.querySelectorAll("[data-i18n]").forEach(function (el) {
    ...
    storeLang(lang);
    // Keep ?lang= in sync without adding history entries
    try {
  ...
  function syncSwitchers(lang) {
    // Every language button in header / drawer / footer
  ...
  /* The header switcher is a segmented control: one dark pill that slides
     (with a little spring) under whichever language is active. Its position
     is measured, not hard-coded, so it survives font swaps and RTL. */
  var pillReady = false;
  function movePill() {
  ...
  // Drop the drawer if the viewport grows past the mobile breakpoint
  ...
  /* ===================================================================
     4. SCROLL STATE
        One rAF-throttled handler drives everything that reacts to scroll:
        the header's compact state, the back-to-top progress ring, and the
        process timeline's light beam.
     ================================================================ */
  ...
  /* ===================================================================
     ...
     Reveals are a clip-wipe (not a plain fade). Elements that enter together
     cascade by 80ms so a row of cards arrives as a sequence. Once an element
     has finished revealing it is marked .is-done, which hands its transitions
     back to the component's own hover styles.
     ================================================================ */
  ...
  (function reveals() {
    ...
    // Last-resort net: only if the observer never reported at all (broken/blocked).
    // "Nothing is on screen yet" is NOT a failure — on a page whose hero fills the
    // viewport that is the normal state, and must keep waiting for the scroll.
    window.setTimeout(function () { if (!observerAlive) showAll(); }, 3000);
  })();

  /* Section headings get a single light sweep as they scroll into view. */
  (function polish() {
  ...
  /* ===================================================================
     6. POINTER EFFECTS — cursor-tracked glare on panels, and a soft 3D tilt
        on the service cards. Skipped on touch and for reduced motion.
     ================================================================ */
  ...
  /* ===================================================================
     6b. SCROLL-DRIVEN "HOVER" — on phones and tablets there is no hover, so
         each card gets .scroll-lit while it crosses the middle of the screen
         and plays the same animation the desktop hover does. The matching
         CSS is generated by _source/build_css.py.
     ================================================================ */
  ...
  /* ===================================================================
     ...
     There is no server behind this site, so rather than pretend to send,
     the form composes the message and opens WhatsApp. This works on a
     plain static host without pretending there is a server-side mail system.
     ================================================================ */
  ...
  (function contactForm() {
    ...
    function setError(field, hasError) {
      ...
      if (hasError) {
        // re-add the class so the shake replays on every failed attempt
  ...
  // Entrance animations are keyed to html:not(.is-settled); once the load-in
  // sequence has played, drop them so a language change can never replay it.
